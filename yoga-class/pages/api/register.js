...
const register = async (req, res) => {
    if (req.method === 'POST') {
        ...
        const { name, batch, email, age } = req.body
        ...
        if (email && name && batch && age) {
            try {
                ...
                if (valAge < 18 || valAge > 65) {
                    ...
                    return res.status(200).json({ msg: "Only 18-65 Age group can enroll",_id:"4" });
                }
                ...
                return res.status(200).json({msg:"Registration Successfyll", _id: "2" });
            } catch (error) {
